...
const PROOFS = ['connectors', 'agents', 'governance', 'measurement'] as const;

export default function FitForAI() {
  const t = useTranslations('fitForAI');

  return (
    <section id="fit-for-ai" className="bg-paper py-24">
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {PROOFS.map((p, i) => (
            ...
            >
              ...
              <h3 className="text-h3 text-ink-700">{t(`items.${p}.title`)}</h3>
              <p className="flex-1 text-body text-slate-500">{t(`items.${p}.body`)}</p>
            </article>
          ))}
        </div>
      </div>
